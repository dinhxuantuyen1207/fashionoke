    ...
    <div>
        {{-- <div>
            <h6><span><a href="/" style="color: #000">Trang Chủ</a></span><small><i
                        ...
                        class="bx bx-right-arrow-alt"></i></small><span><a id="name_head_danh_muc" data-id="{{$data->id}}"
                        href="/{{ $datacha->slug_danh_muc }}/{{ $data->slug_danh_muc }}"
        ...
        <div class="card">
            ...
            @if ($x > 0)
                <div class="card-header mt-3">
                    <h4 class="ms-5" style="text-shadow: 1px 0px rgb(252, 67, 67);color:#e2992b33">Có {{ $x }}
                        Sản Phẩm Được Tìm Thấy</h4>
                </div>
            @endif
            <div class="card-body">
                <div class="mb-4">
                        ...
                        @foreach ($data as $key => $value)
                            <div class="col-md-2 mb-item">
                                <a href="/product/{{ $value->slug_san_pham }}" class="card">
                                    <img src="/upload/{{ $value->hinh_anh_phu }}" class="card-img-top" alt="..."
                                        style="height: 110px;">
            ...
            $.ajax({
                ...
                success : function(res){
                    ...
                    $.each(res.data, function (key, value) {
                        ...
                        noi_dung_select += '<a href="/product/'+value.slug_san_pham+'" class="card">';
                        noi_dung_select += '<img src="/upload/'+value.hinh_anh_phu+'" class="card-img-top" alt="..."';
                        noi_dung_select += 'style="height: 110px;">';
                        if(value.khuyen_mai >= 10){
                            noi_dung_select += '<div class="">';
                            noi_dung_select += '<div class="spin position-absolute top-0 end-0 product-discount "';
                            noi_dung_select += 'style="top:-17px !important;right: -12px !important;width: 44px;height: 44px; background: conic-gradient(red, yellow, lime, aqua, blue, magenta, red)">';
                            noi_dung_select += '</div>';
                            noi_dung_select += '<div class="position-absolute top-0 end-0 product-discount"';
                            noi_dung_select += 'style="top:-15px !important;right: -10px !important;"><span';
                            noi_dung_select += 'class="">-'+value.khuyen_mai+'%</span></div>';
                            noi_dung_select += '</div>';
                        } else if(value.khuyen_mai > 0) {
                            noi_dung_select += '<div class="">';
                            noi_dung_select += '<div class="position-absolute top-0 end-0 product-discount "';
                            noi_dung_select += 'style="top:-17px !important;right: -12px !important;width: 44px;height: 44px; background: conic-gradient(red, yellow, red)">';
                            noi_dung_select += '</div>';
                            noi_dung_select += '<div class="position-absolute top-0 end-0 product-discount"';
                            noi_dung_select += 'style="top:-15px !important;right: -10px !important;"><span';
                            noi_dung_select += 'class="">-'+value.khuyen_mai+'%</span></div>';
                        ...
                        noi_dung_select += '<p class="card-title cursor-pointer text-center"';
                        noi_dung_select += 'style="height: 40px;overflow: hidden;color:rgb(41, 28, 28);">';
                        noi_dung_select += ''+value.ten_san_pham+'</p>';
                        noi_dung_select += '<div class="clearfix">';
                        noi_dung_select += '<p class="mb-0 float-end fw-bold me-1 mb-1" style="font-size: 10px"><span';
                        noi_dung_select += ' class="me-1 text-decoration-line-through text-secondary">';
                        if(value.khuyen_mai > 0){
                            noi_dung_select += ''+value.gia.toLocaleString('vi-VN', {style: 'currency', currency: 'VND'}) +'';
                        }else {

                        }
                        noi_dung_select += '</span><span>'+ (value.gia - value.gia * value.khuyen_mai * 0.01).toLocaleString('vi-VN', {style: 'currency', currency: 'VND'}) +'</span>';
                        noi_dung_select += '</p>';
                        noi_dung_select += '</div>';
                        noi_dung_select += '</a>';
                        noi_dung_select += '</div>';
